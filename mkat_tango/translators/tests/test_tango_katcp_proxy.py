...
from mkat_tango.translators.tango_katcp_proxy import (
    TangoDeviceServerBase, add_tango_server_attribute_list, create_command2request_handler, get_katcp_request_data,
    get_tango_device_server, remove_tango_server_attribute_list,)
from mkat_tango.translators.tests.test_tango_inspecting_client import ClassCleanupUnittestMixin
...
sensors = {
    ...
    "actual-azim": Sensor.float(
        "actual-azim", "Actual azimuth position", "deg", [-183.32, 275.0], 32.9
    ),
    "invalid-float": Sensor.float("invalid-float", "Float with bad min/max", "", [0, 0]),
...
default_attributes = {
    "State",
    "Status",
    "NumErrorTranslatingSensors",
    "ErrorTranslatingSensors",
    "Replies",
    "Informs",
}
...
class _test_KatcpTango2DeviceProxy(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.addCleanup(reset_katcp_tango_server, self.katcp_server, self.instance)
...
class _test_KatcpTango2DeviceProxyCommands(ClassCleanupUnittestMixin, unittest.TestCase):
    longMessage = True
    ...
    def setUpClassWithCleanup(cls):
        ...
        with mock.patch(
            ...
            cls.tango_context = DeviceTestContext(
                cls.TangoDeviceServer, db=cls.tango_db, properties=cls.properties
            )
...
class test_KatcpTango2DeviceProxy(_test_KatcpTango2DeviceProxy):
    def test_connections(self):
        ...
        self.assertEqual(
            is_proxy_connecting_to_server,
            True,
            "No connection established between client and server",
        )
        self.assertEqual(
            self.katcp_ic.is_connected(),
            True,
            "The KATCP inspecting client is not connected to the device" " server",
        )
    ...
    def test_update_tango_server_attribute_list(self):
        ...
        self.assertNotEquals(
            device_attrs, default_attributes, "Attribute list was never updated"
        )
        # Test if the number of attributes has increased after the update
        self.assertGreater(
            len(device_attrs), len(default_attributes), "Attribute list was never updated"
        )
    ...
    def test_sensor_translation_errors(self):
        ...
        self.assertEqual(
            sorted(self.tango_dp.ErrorTranslatingSensors), sorted(invalid_sensor_names)
        )
    ...
    def test_expected_sensor_attributes(self):
        ...
        self.assertEqual(
            set(attr_list),
            set(sensname2tangoname_list),
            "The attribute" " list and the the sensor list do not match",
        )
    ...
    def test_attribute_sensor_properties_match_direct_trans(self):
        ...
        for sensor in list(self.katcp_server._sensors.values()):
            ...
            self.assertEqual(
                tangoname2katcpname(attr_desc.name),
                sensor.name,
            ...
            # type once solution for DevEnum is found.
                ...
                self.assertEqual(
                    sensor.params, [], "The sensor object has a non-empty params list"
                )
    ...
    def test_sensor2attr_removal_updates(self):
        ...
        self.assertIn(
            sensor_name, list(self.katcp_server._sensors.keys()), "Sensor not in" " list"
        )
        self.assertIn(
            katcpname2tangoname(sensor_name),
    ...
    def test_sensor2attr_addition_updates(self):
        ...
        """
        initial_tango_dev_attr_list = set(list(self.tango_dp.get_attribute_list()))
        sens = Sensor(Sensor.FLOAT, "experimental-sens", "A test sensor", "", [-1.5, 1.5])
        self.assertNotIn(
            sens.name,
            list(self.katcp_server._sensors.keys()),
            "Unexpected" " sensor in the sensor list",
        )
        self.assertNotIn(
            katcpname2tangoname(sens.name),
            initial_tango_dev_attr_list,
        ...
        for sensor in katcp_device_server.get_sensors():
            if sensor.stype in ["integer"]:
                ...
                self.assertNotEqual(
                    sensor.value(),
                    value,
                    "Sensor {} value is identical to the value to be set".format(
                        sensor.name
                    ),
                )
    ...
    def _wait_for_tango_attribute_to_update(
        ...
        while value is None:
            ...
            if time.time() > stoptime:
                raise RuntimeError(
                    "TimeOutError : Tango device server not well"
    ...
    def test_sensor_attribute_value_update(self):
        ...
        """
        katcp_device_server = self._update_katcp_server_sensor_values(self.katcp_server)
        for sensor in katcp_device_server.get_sensors():
            if sensor.name in invalid_sensor_names:
                continue
...
class test_KatcpTango2DeviceProxyCommands(_test_KatcpTango2DeviceProxyCommands):
    def test_inspection_of_requests(self):
        ...
        self.assertEqual(is_katcp_server_running, True, "Katcp device server not running")
        ...
        command_list = [command.cmd_name for command in self.device.command_list_query()]
