...
KATCP_REQUEST_DOC_TEMPLATE = """?{desc.cmd_name} {desc.in_type} -> {desc.out_type}
...
TANGO2KATCP_TYPE_INFO = {
    DevFloat: KatcpTypeInfo(
        KatcpType=kattypes.Float,
        sensor_type=Sensor.FLOAT,
        params=dtype_params(np.float32),
    ),
    DevDouble: KatcpTypeInfo(
        KatcpType=kattypes.Float,
        sensor_type=Sensor.FLOAT,
        params=dtype_params(np.float64),
    ),
    DevUChar: KatcpTypeInfo(
        KatcpType=kattypes.Int, sensor_type=Sensor.INTEGER, params=dtype_params(np.uint8)
    ),
    DevShort: KatcpTypeInfo(
        KatcpType=kattypes.Int, sensor_type=Sensor.INTEGER, params=dtype_params(np.int16)
    ),
    DevUShort: KatcpTypeInfo(
        KatcpType=kattypes.Int, sensor_type=Sensor.INTEGER, params=dtype_params(np.uint16)
    ),
    DevLong: KatcpTypeInfo(
        KatcpType=kattypes.Int, sensor_type=Sensor.INTEGER, params=dtype_params(np.int32)
    ),
    DevULong: KatcpTypeInfo(
        KatcpType=kattypes.Int, sensor_type=Sensor.INTEGER, params=dtype_params(np.uint32)
    ),
    DevLong64: KatcpTypeInfo(
        KatcpType=kattypes.Int, sensor_type=Sensor.INTEGER, params=dtype_params(np.int64)
    ),
    DevULong64: KatcpTypeInfo(
        KatcpType=kattypes.Int, sensor_type=Sensor.INTEGER, params=dtype_params(np.uint64)
    ),
    DevBoolean: KatcpTypeInfo(
        KatcpType=kattypes.Bool, sensor_type=Sensor.BOOLEAN, params=()
    ),
    DevString: KatcpTypeInfo(
        KatcpType=kattypes.Str, sensor_type=Sensor.STRING, params=()
    ),
    DevEnum: KatcpTypeInfo(
        KatcpType=kattypes.Discrete, sensor_type=Sensor.DISCRETE, params=()
    ),
...
def tango_attr_descr2katcp_sensors(attr_descr):
    ...
    if (
        attr_descr.data_format != AttrDataFormat.SCALAR
        and attr_descr.data_format != AttrDataFormat.SPECTRUM
    ):
...
def tango_cmd_descr2katcp_request(tango_command_descr, tango_device_proxy):
    ...
    tango_request = partial(
        tango_device_proxy.command_inout, green_mode=tango.GreenMode.Futures, wait=False
    )
...
def tango_type2kattype_object(tango_type):
    ...
    try:
        ...
        raise NotImplementedError(
            "Tango wrapping not implemented for tango type {}".format(tango_type)
        )
...
class TangoDevice2KatcpProxy(object):
    ...
    def update_request_sensor_list(
    ...
    def update_katcp_server_sensor_list(self, attributes):
        ...
        for attribute_name, attribute_config in attributes.items():
            ...
            if attribute_config.data_format == AttrDataFormat.SPECTRUM:
                ...
                self._logger.info("sensor_name: %s", sensor_name)
                self._logger.info("xxxxx sensors and sensors_: %s %s", sensors, sensors_)
        ...
        self._logger.info("sensors_to_add: %s", sensors_to_add)
    ...
    def update_katcp_server_request_list(self, commands):
        ...
        for request_name in requests_to_add:
            try:
                ...
                req_handler = self._dummy_request_handler_factory(request_name, str(exc))
...
def tango2katcp_main(args=None, start_ioloop=True):
    ...
    parser = ArgumentParser(description="Launch Tango device -> KATCP translator")
    parser.add_argument(
        "-l",
        "--loglevel",
        default="INFO",
    ...
    parser.add_argument(
        "--katcp-server-address",
        type=address,
        help="HOST:PORT for the device to listen on",
        required=True,
    )
    parser.add_argument(
        "tango_device_address",
        type=str,
