...
alarm_list = {
    'AlarmList': ['ANC_Wind_Speed:(mkat/proxies/anc/mean_wind_speed.value > 11.1 or '
                  ...
                  'AGG_System_Fire_Ok:(mkat/proxies/anc/bms_kapb_fire_active.value '
                      '== True)',
                  'AGG_System_Power_Ok:(mkat/proxies/anc/bms_imminent_power_failure '
                      '== True)',
...
alarm_descriptions = {
    'AlarmDescriptions': ['ANC_Wind_Speed:ANC implements this sensor giving 10-min '
                          'mean wind speed over all wind sensors',
                          'ANC_Wind_Gust:ANC implements this sensor giving highest '
                          'gust over all wind sensors',
                          'AGG_System_Fire_Ok:True if there is no fire detected '
                          'in KAPB',
                          'AGG_System_Power_Ok:True if there is no power supply '
                          'voltage in the KAPB',
                          'AGG_Wind_Speed_reporting:True if all wind related sensors ok',
                       ]
}
alarm_receivers = {
    'AlarmReceivers': ['ANC_Wind_Speed:cam@ska.ac.za',
...
alarm_severities = {
    'AlarmSeverities': ['ANC_Wind_Speed:ALARM',
                        'ANC_Wind_Gust:ALARM',
